...
class BDD100KToYOLOConverter:
    ...
    def validate_annotations(self, item):
        ...
        if labels is None:  # Explicit check for None
            return False
        return any(isinstance(ann, dict) and 
                'box2d' in ann and 
                ann.get('category') in self.label_map 
                for ann in labels)


    def process_dataset(self, image_dir, label_path, output_dir, split='train', max_samples=7000, num_verify_samples=3):
...
def main():
    ...
    # Path configuration
    BASE_DIR = "C://Users//User1//EdgeAI-BDD100K-FPGA-NN//archive"
    ...
    OUTPUT_DIR = "C:/Users/User1/EdgeAI-BDD100K-FPGA-NN/bdd100k_yolo_640_100k_v5"
    # OUTPUT_DIR = "C:/Users/User1/EdgeAI-BDD100K-FPGA-NN/YOLO/bdd100k_yolo_640"
    
    # Process datasets
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    print("=== Processing Training Set ===")
    converter.process_dataset(TRAIN_DIR, TRAIN_LABEL_PATH, OUTPUT_DIR, split='train', max_samples=7000)
    ...
    yaml_content = {
        ...
        'train': 'images/train',
        'val': 'images/val',
        'test': 'images/test',  # Added test set path
        'names': converter.idx_to_class,
        'nc': len(converter.idx_to_class)
    }
    config_path = os.path.join(OUTPUT_DIR, 'bdd100k_yolo.yaml')
