...
def postprocessing(x, nc=10):
    print("Input x type:", type(x), "len:", len(x))
    ...
    anchors = anchors.float().view(3, -1, 2)

    for i in range(3):
        print("x[{}] shape:".format(i), x[i].shape)
        bs, na, ny, nx, no = x[i].shape  # Expect [bs, na, ny, nx, 5+nc]
        if no != 5 + nc:
            print(f"Warning: x[{i}] has {no} channels, expected {5+nc}")
        if grid[i].shape[2:4] != (ny, nx):
            grid[i], anchor_grid[i] = make_grid(nx, ny, i, anchors, stride)
            print("grid[{}] shape:".format(i), grid[i].shape)
        xy, wh, conf = x[i].sigmoid().split((2, 2, 1 + nc), 4)
        xy = (xy * 2 + grid[i]) * stride[i]
        wh = (wh * 2) ** 2 * anchor_grid[i]
        y = torch.cat((xy, wh, conf), 4)
        print("y[{}] shape:".format(i), y.shape)
        z.append(y.view(bs, na * ny * nx, 5 + nc))
        print("z[{}] shape:".format(i), z[-1].shape)
    output = torch.cat(z, 1)
    print("Final output shape:", output.shape, "type:", type(output))
    return output, x
